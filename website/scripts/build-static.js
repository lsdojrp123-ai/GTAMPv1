...
const samplePosts = posts.length ? posts : [
  { id: 1, cat: 'announcements', title: 'GTAMP v1.9.0 released', body: 'In-game connect panel, F8 console, T chat — the full FiveM-style join flow is live.', uid: 1, at: Date.now() },
...
(async () => {
  ...
  try {
    const exe = path.join(__dirname, '..', '..', 'GTAMP-Launcher-v1.9.0.exe');
    if (fs.existsSync(exe)) fs.copyFileSync(exe, path.join(outDir, 'GTAMP-Launcher-v1.9.0.exe'));
  } catch (e) {}
  fs.writeFileSync(path.join(outDir, 'sample-servers.json'), JSON.stringify({ servers: live() }));
  fs.writeFileSync(path.join(outDir, 'launcher-version.json'), JSON.stringify({ version: '1.9.0', url: '/download/GTAMP-Launcher-v1.9.0.exe' }));

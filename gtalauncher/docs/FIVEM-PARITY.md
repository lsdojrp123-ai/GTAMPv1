...
| Crash/diag culture (`CfxCrashDump`, in-UI fatal cards with cause text) | **v1.9.9: observable failure** — injector heartbeats stream onto the connect card live; every stage/error line is ring-buffered and auto-copied to the clipboard on failure; blocked injector (av/SmartScreen), elevated game (access-denied → one-click admin relaunch), and real inject failures each get their own exact card, in seconds, with the cause in plain language |
